...
// 椰子
//
// 広葉樹の枝分かれを流用すると、幹の先で Y 字が増える「苗木」にしか見えない。
// 椰子のシルエットは、一本の曲がった幹と、頂点から放射状に垂れる葉で作る。
// ---------------------------------------------------------------------------
...
function palmFrondGeometry(
  ...
  for (let i = 0; i < SEGMENTS; i++) {
    const a = rings[i];
    const b = rings[i + 1];
    // 地上から見上げても、上空から見下ろしても葉が消えないよう両面を持たせる。
...
export function buildPalm(seed: number): THREE.BufferGeometry {
  ...
  // branchGeometry の曲がった中心線の先端。幹を回した分も同じように回す。
  const crown = new THREE.Vector3(curve * height, height, 0).applyAxisAngle(UP, trunkYaw);
  parts.push(
    paint(
      new THREE.IcosahedronGeometry(0.3, 0).translate(crown.x, crown.y, crown.z),
      0x526735,
    ),
  );
  ...
  for (let i = 0; i < count; i++) {
    ...
    frond.rotateY(baseYaw + (i / count) * Math.PI * 2 + rr(-0.16, 0.16));
